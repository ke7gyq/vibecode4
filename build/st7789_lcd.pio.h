...
#define st7789_lcd_16_wrap_target 0
#define st7789_lcd_16_wrap 1
#define st7789_lcd_16_pio_version 0

static const uint16_t st7789_lcd_16_program_instructions[] = {
            //     .wrap_target
    0x6001, //  0: out    pins, 1         side 0
    0xb042, //  1: nop                    side 1
            //     .wrap
...
#if !PICO_NO_HARDWARE
static const struct pio_program st7789_lcd_16_program = {
    .instructions = st7789_lcd_16_program_instructions,
    .length = 2,
    .origin = -1,
    .pio_version = st7789_lcd_16_pio_version,
#if PICO_PIO_VERSION > 0
    .used_gpio_ranges = 0x0
#endif
};

static inline pio_sm_config st7789_lcd_16_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + st7789_lcd_16_wrap_target, offset + st7789_lcd_16_wrap);
    sm_config_set_sideset(&c, 1, false, false);
    return c;
}
...
// 16-bit version: accepts 16-bit big-endian words from DMA with byte-swap enabled
static inline void st7789_lcd_16_program_init(PIO pio, uint sm, uint offset, uint data_pin, uint clk_pin, float clk_div) {
    pio_gpio_init(pio, data_pin);
    pio_gpio_init(pio, clk_pin);
    pio_sm_set_consecutive_pindirs(pio, sm, data_pin, 1, true);
    pio_sm_set_consecutive_pindirs(pio, sm, clk_pin, 1, true);
    pio_sm_config c = st7789_lcd_16_program_get_default_config(offset);
    sm_config_set_sideset_pins(&c, clk_pin);
    sm_config_set_out_pins(&c, data_pin, 1);
    sm_config_set_fifo_join(&c, PIO_FIFO_JOIN_TX);
    sm_config_set_clkdiv(&c, clk_div);
    sm_config_set_out_shift(&c, false, true, 16);  // 16-bit autopull threshold
    pio_sm_init(pio, sm, offset, &c);
    pio_sm_set_enabled(pio, sm, true);
}
...
// 16-bit put for use with DMA
static inline void st7789_lcd_put_16(PIO pio, uint sm, uint16_t x) {
    while (pio_sm_is_tx_fifo_full(pio, sm))
        ;
