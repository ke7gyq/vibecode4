...
int waterfall_accm_add_fft(waterfall_accm_t *accm, const q15_t *fft_output, uint32_t num_outputs)
{
    ...
    for (uint32_t bin = 0; bin < num_outputs && fft_idx < WATERFALL_FFT_BINS; bin++) {
        ...
        /* DEBUG: Log first frame's bin values (DISABLED - causes stack overflow) */
        /* if (getFFTDebug() && accm->frame_count == 0 && bin < 16) {
            printf("[FFT] Bin %u: real=%d, imag=%d, mag_sq=%lu\n", 
                   bin, (int)real, (int)imag, (uint32_t)mag_sq);
        } */
